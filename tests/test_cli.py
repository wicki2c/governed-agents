...
def test_proposals_default_query_params(monkeypatch, capsys):
    import httpx
    ...
    assert seen["url"].endswith("/proposals?status=pending&limit=100")


def test_proposals_custom_flags(monkeypatch, capsys):
    import httpx
    ...
    rc = cli.main(["proposals", "--status", "all", "--limit", "20", "--url", "http://127.0.0.1:9000/"])
    assert rc == 0
    out = capsys.readouterr().out
    assert json.loads(out) == [{"proposal_id": "p1"}]
    assert seen["url"] == "http://127.0.0.1:9000/proposals?status=all&limit=20"
